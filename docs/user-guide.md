...
[简体中文](user-guide.md) ｜ [English](user-guide.en.md) ｜ [返回项目首页](../README.md)
...
- 工具默认只监听本机地址，不提供账号、权限、多用户协作、远程访问或公网部署能力。
...
- 结构化日志按已实现字段能力解析，已知诊断字段按实际出现次数统计覆盖率，额外未知字段会忽略。无法识别的行不会进入有效统计。
